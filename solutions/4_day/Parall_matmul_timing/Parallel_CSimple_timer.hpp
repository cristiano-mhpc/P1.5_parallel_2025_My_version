    ...
    */
    ...
    if (some_rank == 0){
      ...
      */
      ...
      //print the timings in the struct Data corresponding to "Total". This is called only once.
      std::cout << std::fixed << std::setprecision(15);
      //std::cout << "Total time: " << table["Total"].timings << std::endl;
      
      //print the total time in the table corresponding in "comm" key.
      //std::cout << "Communication time: " << table["comm"].timings << std::endl;

      //print the computation time
      //std::cout << "Computation time: " << table["Total"].timings - table["comm"].timings << std::endl;
      ...
      for(i = 0; i < num_funcs; i++){
          //std::cout << max[i] << " ";
          std::cout << scope_names[i] << " " << max[i] << std::endl;
      }
      std::cout << std::endl;
      */

      //print the total time
      std::cout << scope_names[0] << " " << max[0] << std::endl;

      //print the commmunication time 
      std::cout << scope_names[1] << " " << max[1]*table["comm"].num_calls << std::endl;

      //the computation time is the difference
      std::cout << "Computation time: " << max[0]-(max[1]*table["comm"].num_calls) << std::endl;

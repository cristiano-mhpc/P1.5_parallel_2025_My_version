...
double critical(double x0, double x1, int steps, double(*func)( double)){
  double approx{0.0};
  double base = (x1-x0)/steps;
  int i;
  {  
    CSimple_timer t{"critical"};
...
double atomic(double x0, double x1, int steps, double(*func)( double)){
  double approx{0.0};
  double base = (x1-x0)/steps;
  int i;
  {  
    CSimple_timer t{"atomic"};
...
int main(int argc, char** argv){
  ...
  double approx2 = atomic(x0, x1, steps, func);

  double appro3 = critical(x0, x1, steps, func);

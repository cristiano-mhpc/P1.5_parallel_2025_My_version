...
void fft_3d_slice(fftw_handler* fft, fftw_handler* fft_1d_h, fftw_handler* fft_2d_h, int n1, int n2, int n3, 
                  double *data_direct, fftw_complex* data_rec, bool direct_to_reciprocal)
{
    ...
    if ( direct_to_reciprocal) {
        ...
        //perfrom 2d fft on complex data slices
        for (int i1 = 0; i1 < n1; i1++) {
            // take a cslice of data_rec
            fillCSlice(data_rec,cslice,i1,n2,n3);
            // fill the 2D FFT structure with cslice
            for(int k = 0; k < n2*n3; k++) {
	              fft_2d_h->fftw_data[k]  = cslice[k];
	          }
            // execute backward 2D fftw
            fftw_execute_dft(fft_2d_h->bw_plan, fft_2d_h->fftw_data, fft_2d_h->fftw_data);

            // write results back to cslice
            for(int k = 0; k < n2*n3; k++) {
	              cslice[k] = fft_2d_h->fftw_data[k];
	          }
            //memcpy(cslice, fft_2d_h->fftw_data, n2*n3*sizeof(fftw_complex));
            //push cslice back to data_c
            pushCSlice(data_rec,cslice,i1,n2,n3);
        }

        //perform 1D dft along i1 of data_direct
        for (int i2 = 0; i2 < n2; i2++) {
          for (int i3 = 0; i3 < n3; i3++) {
              // take a Cline of data
              fillCLine(data_rec,cline,i2,i3,n1,n2,n3);
              ...
              // execute 1D bw fftw
              fftw_execute_dft(fft_1d_h->bw_plan, fft_1d_h->fftw_data, fft_1d_h->fftw_data);
              memcpy(cline, fft_1d_h->fftw_data, n1*sizeof(fftw_complex));
              pushCLine(data_rec,cline,i2,i3,n1,n2,n3);
          }

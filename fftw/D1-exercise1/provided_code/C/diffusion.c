...
int main(int argc, char **argv) {
  ...
  plot_data_2d("diffusivity", n1, n2, n3, (int)(fft_h.local_n1),
               (int)(fft_h.local_n1_offset), 1, diffusivity);
  plot_data_2d("diffusivity", n1, n2, n3, (int)(fft_h.local_n1),
               (int)(fft_h.local_n1_offset), 2, diffusivity);
  plot_data_2d("diffusivity", n1, n2, n3, (int)(fft_h.local_n1),
               (int)(fft_h.local_n1_offset), 3, diffusivity);
  ...
  double ss_temp = ss;
  MPI_Allreduce(&ss_temp, &ss, 1, MPI_DOUBLE, MPI_SUM, fft_h.mpi_comm);
  ss = 1.0 / (ss * fac);
  ...
  for (istep = 1; istep <= nstep; ++istep) {
    ...
    if (istep % 30 == 1) {
      ...
      ss_temp = ss;
      MPI_Allreduce(&ss_temp, &ss, 1, MPI_DOUBLE, MPI_SUM, fft_h.mpi_comm);
            double r2mean_temp = r2mean;
      MPI_Allreduce(&r2mean_temp, &r2mean, 1, MPI_DOUBLE, MPI_SUM, fft_h.mpi_comm);

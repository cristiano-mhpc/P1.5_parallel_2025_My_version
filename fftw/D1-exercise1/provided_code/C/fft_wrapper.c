...
void init_fftw(fftw_mpi_handler *fft, int n1, int n2, int n3,
   ...
   */
  fftw_mpi_init();
  fft->mpi_comm = mpi_comm; //<----my edit

  /*
   *  Allocate a distributed grid for complex FFT using aligned memory
   * allocation See details here:
   *  http://www.f&fft_hftw.org/fftw3_doc/Allocating-aligned-memory-in-Fortran.html#Allocating-aligned-memory-in-Fortran

...
bool compareMatrices(const std::vector<std::vector<double>>& M1, const std::vector<std::vector<double>>& M2, double tolerance = 1) {
    ...
    for (size_t i = 0; i < M1.size(); ++i) {
        for (size_t j = 0; j < M1[0].size(); ++j) {
            if (std::abs(M1[i][j] - M2[i][j]) > tolerance) {
                std::cout<<M1[i][j]<<" "<<M2[i][j]<<" "<<std::abs(M1[i][j] - M2[i][j])<<std::endl;
                return false;
            }
        }
    }
